...
<template>
    <div
        class="relative w-4/5 mx-auto mt-[7.6rem] z-10 text-lg font-medium text-job-darkCyan bg-white blueShadow rounded-[0.25rem]">
        ...
        <ul class="flex min-h-[4rem] gap-6 px-8 py-4 flex-wrap">
            <li v-for="(item, index) in picked" :key="index"
                class="flex items-center h-[2rem] pl-2 bg-red-100 bg-job-lightGrayishCyan rounded-[0.25rem] overflow-hidden">
                {{  item  }}
    ...
    <div id="drop" class="relative w-4/5 mx-auto mt-4 z-10 bg-white rounded-[0.25rem] p-2" v-if="isOpen" tabindex="0">
